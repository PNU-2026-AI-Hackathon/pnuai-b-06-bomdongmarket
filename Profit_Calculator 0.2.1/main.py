"""Profit Calculator 0.2.1 계산 실행 및 콘솔 진입점."""
...
def calculate_all_sites() -> list[dict[str, object]]:
    """CSV를 읽어 모든 사업장의 1~10번 계산 결과를 반환한다."""
    ...
    for space_row in spaces:
        crop_name = space_row["crop_name"].strip()
        if crop_name not in crops or crop_name not in sales_info:
            raise KeyError(f"작물 정보 또는 판매가격이 없습니다: {crop_name}")

        crop = crops[crop_name]
        space_result = calculate_space(space_row)
        production_result = calculate_production(space_result, crop)
        sales_result = calculate_sales(production_result, sales_info[crop_name])
        ...
        labor_result = calculate_labor_cost(production_result, crop, standard)
        profit_result = calculate_profit(
            ...
            monthly_water_cost_krw=water_result["monthly_water_cost_krw"],
            monthly_material_cost_krw=material_result[
                "monthly_material_cost_krw"
            ],
            monthly_labor_cost_krw=labor_result["monthly_labor_cost_krw"],
            standard=standard,
            contract=contract,
        )
