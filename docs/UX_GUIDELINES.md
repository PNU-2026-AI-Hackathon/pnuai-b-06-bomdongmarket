...
- 실재 여부를 검증해야 하는 값(주소 등)은 자유 입력 대신 외부 검색 결과로만 채우고, 검색에 없는 나머지(동·호수 등)는 별도 필드로 받는다. 고른 값은 지도처럼 확인 가능한 형태로 되짚어 준다.
- 외부 SDK가 필요한 보조 기능(지도 미리보기 등)은 키가 없거나 로드에 실패해도 폼 제출까지의 흐름을 막지 않고 안내 문구로 대체한다.
