...
- 목적: 대시보드에서 받은 신청과 보낸 신청을 한 번에 확인하고, 받은 신청을 처리하는 화면 전용 관리 모달이다.
- 범위: 현재는 대시보드 전용 컴포넌트로 유지한다. 같은 정보 관리 패턴이 다른 화면에도 필요해질 때 공통 Dialog primitive로 승격한다.
- ConfirmDialog와의 구분: 단일 결정을 확인하는 팝업이 아니라 여러 신청의 상태와 action을 제공하므로 ConfirmDialog를 재사용하지 않는다.
- 상태: 받은 신청과 보낸 신청은 각각 빈 상태를 제공하고, 처리 오류는 모달 안에서 해당 작업과 가까운 role=alert로 알린다.
