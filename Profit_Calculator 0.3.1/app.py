"""Profit Calculator 0.3.1 Streamlit 대시보드."""
...
st.set_page_config(
    page_title="Profit Calculator 0.3.1",
    page_icon="🌿",
    layout="wide",
...
st.markdown(
    f"""
    <div class="hero">
        <div class="hero-kicker">Profit Calculator · Version 0.3.1</div>
...
st.write("")
tabs = st.tabs(["3×3 비교", "계약 추천", "월별 에너지", "계산 상세", "입력 데이터"])
...
with tabs[0]:
    ...
    with left:
        ...
        st.bar_chart(landlord_matrix.round(0), use_container_width=True)
    with right:
        st.markdown("#### 사업장 영업이익")
        business_matrix = summary.pivot(index="사업장", columns="작물", values="사업장 영업이익(원)")
        st.bar_chart(business_matrix.round(0), use_container_width=True)
...
with tabs[1]:
    ...
    with st.expander("단기계약형 예상 영업구조", expanded=False):
        st.markdown(
            ...
            현재 0.3.1은 단기계약형의 구체적인 추가 매출·비용을 계산하지 않고 추천 결과까지만 제공합니다.
            """
        )
...
with tabs[2]:
    ...
    st.line_chart(energy.set_index("월")[energy_columns], use_container_width=True)
...
with tabs[3]:
    ...
    for block in detail["계산 블록"].drop_duplicates():
        ...
        with st.expander(str(block), expanded=block in {"1. 공간", "10. 수익"}):
            st.dataframe(block_rows[["항목", "결과"]], use_container_width=True, hide_index=True)
